...
const MedicalReport = () => {
  const { t } = useTranslation();
	...
	async function getChat(e) {
		...
		let requestData = {
			query: input,
			file_name:"CTScan.txt"
			
		};
		
		

		const url = "https://53d2-117-236-190-193.ngrok-free.app/check";
		try {
			...
			if (response.ok) {
				...
				console.log("Bot:", responseData.result);
				setChatLog([
					...chatLogNew,
					{ user: "gpt", message: responseData.result },
				]);
			} else {
			...
			<section>
				<div className="text-center mt-[-80px] flex lg:w-full  bg-[#282c34] text-white top-0 bottom-0 left-0 right-0 h-[80vh]">
					...
					<section className="flex-1 bg-[rgb(52,53,65)] relative">
						...
						<div className=" p-[24px] items-center justify-between absolute bottom-0 left-0 right-0">
            <div>
